...
bool Drawer::drawModManager = false;
bool Drawer::drawDLCs = false;
void Drawer::mainMenu(LPDIRECT3DDEVICE9 pDevice)
{
...

	{
...

		{
			...
			int current = modManager::mods.list[modManager::mods.selected]->cfgFile;;
			if (ImGui::Button("select config file"))
				ImGui::OpenPopup("config_file_popup");
			if (ImGui::IsItemHovered())
				ImGui::SetTooltip(Startup::cfg.modCFGNameTip.c_str());
			ImGui::SameLine();
			ImGui::TextUnformatted(Startup::cfg.modCFGName);
			if (ImGui::BeginPopup("config_file_popup"))
			{
				for (int i = 0; i < modManager::mods.list[modManager::mods.selected]->cfgFiles.size(); i++)
				{
					if (ImGui::Selectable(modManager::mods.list[modManager::mods.selected]->cfgFiles[i].c_str()))
					{
						modManager::mods.list[modManager::mods.selected]->cfgFile = i;
						for (int a = 0; a < strlen(Startup::cfg.modCFGName); a++) {
							Startup::cfg.modCFGName[a] = 0;
						}
						strcpy(Startup::cfg.modCFGName, modManager::mods.list[modManager::mods.selected]->cfgFiles[i].c_str());
					}
				}
				ImGui::EndPopup();
			}
		...
		if (ImGui::Button("Start mod"))
		{
			...
			{
				if (Startup::gameConfig.justRun == false)
				{
					injector::start();
				}
			}
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Run mod and apply settings");



		if (modManager::mods.list[modManager::mods.selected]->EOP == 0)
		{
			Startup::gameConfig.justRun = true;
		}
		ImGui::SameLine();
		ImGui::Checkbox("just run", &Startup::gameConfig.justRun);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip(Startup::gameConfig.justRunTip.c_str());
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Run vanilla and apply settings, works with steam version only!");


		if (ImGui::Button("Start dlc"))
		{
			drawDLCs = true;
		}
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Run vanilla or dlc");

		ImGui::SameLine();

		ImGui::Checkbox("Close after game start", &Startup::cfg.closeAfterStart);
		if (ImGui::IsItemHovered())
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Exit and save changes");
		...
		ImGui::Begin("drawManager window", NULL, window_flags);

		if (ImGui::Button("open mod manager"))
		{
			Drawer::drawModManager = true;
		}
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Here you can manage your mods!");
	...
	if (Drawer::drawModManager)
		if (modManager::Draw(pDevice,&Drawer::drawModManager))
		{
			Startup::reloadMod();
		}
...
void Drawer::DLCs(LPDIRECT3DDEVICE9 pDevice)
{
	ImGuiIO& io = ImGui::GetIO();
	ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize;
	...
	ImGui::Text("Select what you want to run:\n\n");

	{
		if (ImGui::Button("Start DLC1"))
		{
		}
	}
	ImGui::SameLine();
	...
	{
		if (ImGui::Button("Start DLC2"))
		{
		}
	}
	ImGui::Text("\n");
	{
		if (ImGui::Button("Start DLC3"))
		{
		}
	}
