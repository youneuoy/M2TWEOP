...
void Startup::writeGameCfg(int ifmod)
{
	...
	while (f1.good()) {
		...
		if (s.find("borderless_window")!=string::npos
			...
			||
			s.find("battle_resolution") != string::npos
			||
			s.find("campaign_resolution") != string::npos
			)
			continue;
		f2 << s << endl;
...
void Startup::setRealDir()
{
	CHAR currentPath[MAX_PATH] = { 0 };
	GetCurrentDirectory(sizeof(currentPath), currentPath);
	Startup::gameInfo.realDir = new char[strlen(currentPath) + 1];
	strcpy(Startup::gameInfo.realDir, currentPath);
}

void Startup::reloadMod()
{
	if (modManager::mods.list.size() == 0)
	{
		MessageBoxA(NULL, "dont find any mod!", "Error", MB_OK | MB_ICONASTERISK);
		exit(0);
	}
	int curMod = modManager::mods.selected;
	string md = modManager::mods.list[curMod]->path;
	if (!SetCurrentDirectory(md.c_str()))
	{
		MessageBoxA(NULL,"Cant change directory!", "Error", MB_OK | MB_ICONASTERISK);
	}
	modManager::writeLastMod(gameInfo.realDir);
	startP();
}
