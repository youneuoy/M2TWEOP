...
void readMF()
{
	...
	std::string gamever;
	std::ifstream f1("runningMod.youneuoycfg");
	...
	getline(f1, modPath);
	f1.close();
	modPath += "\\HotSeatToolLib.dll";
	HANDLE hModule = LoadLibraryA(modPath.c_str());
}

void initNVAPI()
{
	NvAPI_Status nvRet = NVAPI_OK;
	nvRet = NvAPI_Initialize();
	if (nvRet != NVAPI_OK)
	{
		//MessageBoxA(NULL, "NvAPI error", "Attention", NULL);
	}
	readMF();
}
